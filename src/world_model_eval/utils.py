...
def predict(video, trial, n=5, return_raw=False):
...
""".strip()
    ...
    # Resilience knobs:
    # - max_retries=8: SDK default is 2; we want long-running batch jobs to ride
    #   through transient API hiccups (ConnectTimeout, 5xx, 429s) instead of
    #   crashing the whole eval after hours of compute. Exponential backoff is
    #   applied automatically by the SDK.
    # - timeout=300.0: per-request budget (default 600s). 5 min is generous for
    #   gpt-4o vision with n up to ~25 generations.
    client = OpenAI(max_retries=8, timeout=300.0)
    messages = [{
        "role": "user",
    ...
    per_call = []  # only populated when return_raw=True
    ...
    for choice in res.choices:
        ...
        if m:
            val = m.group(1).strip()
            if val in ("0.0", "0"):
                key = "0"; parsed_score = 0.0
            elif val in ("1.0", "1"):
                key = "1"; parsed_score = 1.0
            elif val in ("0.5",):
                key = "0.5"; parsed_score = 0.5
            else:
